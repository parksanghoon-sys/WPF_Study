...
namespace Indigator.Lib
{
    ...
    public readonly struct Part
    {
        ...
        /// <summary>
        /// 파트에 Tranform 을 추가 적용한다
        /// </summary>
        /// <param name="part">파트</param>
        /// <param name="transform">변환</param>
        /// <returns>변환이 적용된 파트</returns>
        public static Part operator *(in Part part, in Transform transform) => new Part(part.Drawing, part.Transform * transform);
        public static Part operator *(in Transform transform, in Part part) => new Part(part.Drawing, part.Transform * transform);
        ...
        public static Part CreateRectangle(in double x, in double y, in double width, in double height, Transform transform)
            => new Part(BasicSquare.Instance, Transform.CreateScaling(width, width).Translate(x, y) * transform);
        /// <summary>
        /// 타원 파트를 생성합니다.
        /// </summary>
        /// <param name="x">X 좌표</param>
